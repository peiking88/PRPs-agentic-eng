...
`unsafe` blocks must be _exceptional_ and extensively documented:
...
| **Native libSQL driver (`libsql‑client` 0.2)** | First‑class async wrapper for SQLite 3.46—ideal for _Rustash_ local DB.                                |
...
_Each crate stays under **200 source lines** per file; split into modules when approaching the limit._
...
_`rustc --deny warnings` is the default in CI._ ([doc.rust-lang.org][3])
...
| Area             | Rule                                                                            |
| ---------------- | ------------------------------------------------------------------------------- |
| **Dependencies** | `cargo audit` must pass — no _RUSTSEC_ advisories in `main` ([rustsec.org][11]) |
...
- **Clippy Pedantic group** is _WARN_ baseline; promote violations to _DENY_ when stable.
- No `unwrap()`, `expect()`, `todo!()` in production code paths.
- Public items _must_ carry rustdoc with examples; `cargo doc --document-private-items` must build cleanly (no `missing_docs`).
...
- [ ] All tests & doc‑tests pass via _nextest_
